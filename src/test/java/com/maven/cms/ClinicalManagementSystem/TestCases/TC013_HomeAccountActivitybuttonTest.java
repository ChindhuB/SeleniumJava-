...
public class TC013_HomeAccountActivitybuttonTest extends baseClass {
	@Test
	public void verifyHomepgAccountActivitybttn() throws IOException, InterruptedException {
		driver.get(url);
		...
		hp.clickAccountactivitybttn();
		if (hp.valAccountactivitybttn()) {
			Assert.assertTrue(true);
			logger.info("Home Page Account activity button verification Passed");
		} else {
			logger.info("Home Page Account activity button verification Failed");
			captureScreen(driver, "hpacctactivitybttnTest");
			Assert.assertTrue(false);
		}
