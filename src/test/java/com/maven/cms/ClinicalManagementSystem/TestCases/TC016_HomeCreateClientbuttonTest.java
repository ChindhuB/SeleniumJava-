...
public class TC016_HomeCreateClientbuttonTest extends baseClass {
	@Test
	public void verifyHomepCreateClientgbttn() throws IOException, InterruptedException {
	driver.get(url);
	...
	hp.clickCreatebttn();
	hp.clickCreateClientbttn();
	if (hp.valCreateClientbttn()) {
		Assert.assertTrue(true);
		logger.info("Home Page Create Client button verification Passed");
	} else {
		logger.info("Home Page Create Client button verification Failed");
		captureScreen(driver, "hpcreateclientbttnTest");
		Assert.assertTrue(false);
	}
