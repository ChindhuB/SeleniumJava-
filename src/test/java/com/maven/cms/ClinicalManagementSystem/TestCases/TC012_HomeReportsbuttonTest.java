...
public class TC012_HomeReportsbuttonTest extends baseClass {
	@Test
	public void verifyHomepgReportsbttn() throws IOException, InterruptedException {
		...
		hp.clickReportsbttn();
		if (hp.valReportsbttn()) {
			Assert.assertTrue(true);
			logger.info("Home Page Reports button verification Passed");
		} else {
			logger.info("Home Page Reports button verification Failed");
			captureScreen(driver, "hpreportsbttnTest");
			Assert.assertTrue(false);
		}
