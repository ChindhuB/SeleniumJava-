...
public class TC021_HomeHelpbutton extends baseClass {
	@Test
	public void verifyHomepgHelpbttn() throws IOException, InterruptedException {
		driver.get(url);
		...
		if (hp.valHelpbttn()) {
			Assert.assertTrue(true);
			logger.info("Home Page Help button verification Passed");
		} else {
			logger.info("Home Page Help button verification Failed");
			captureScreen(driver, "hphelpbttnTest");
			Assert.assertTrue(false);
		}
