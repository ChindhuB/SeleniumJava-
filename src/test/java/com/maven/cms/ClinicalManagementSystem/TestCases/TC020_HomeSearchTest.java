...
public class TC020_HomeSearchTest extends baseClass {
	@Test
	public void verifyHomepgSearch() throws IOException, InterruptedException {
		...
		hp.sendHPSearchvar(ser);
		if (hp.valsendHPSearchvar()) {
			Assert.assertTrue(true);
			logger.info("Home Page Search option verification Passed for :" + ser);
		} else {
			logger.info("Home Page Search option verification Failed for :" + ser);
			captureScreen(driver, "hpsearchoptionTest");
			Assert.assertTrue(false);
		}
