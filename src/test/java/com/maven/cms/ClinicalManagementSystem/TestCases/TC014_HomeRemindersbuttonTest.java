...
public class TC014_HomeRemindersbuttonTest extends baseClass {
	@Test
	public void verifyHomepgRemindersbttn() throws IOException, InterruptedException {
		driver.get(url);
		...
		hp.clickRemindersbttn();
		if (hp.valRemindersbttn()) {
			Assert.assertTrue(true);
			logger.info("Home Page Reminders button verification Passed");
		} else {
			logger.info("Home Page Reminders button verification Failed");
			captureScreen(driver, "hpreminderbttnTest");
			Assert.assertTrue(false);
		}
