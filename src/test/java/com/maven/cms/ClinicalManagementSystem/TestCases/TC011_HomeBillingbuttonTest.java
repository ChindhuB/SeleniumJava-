...
public class TC011_HomeBillingbuttonTest extends baseClass {
	@Test
	public void verifyHomepgBillingbttn() throws IOException, InterruptedException {
		...
		hp.clickBillingbttn();
		if (hp.valBillingbttn()) {
			Assert.assertTrue(true);
			logger.info("Home Page Billing button verification Passed");
		} else {
			logger.info("Home Page Billing button verification Failed");
			captureScreen(driver, "hpbillingbttnTest");
			Assert.assertTrue(false);
		}
