...
public class TC009_HomeClientbuttonTest extends baseClass {
	@Test
	public void verifyHomepgClientbttn() throws IOException, InterruptedException {
		driver.get(url);
		...
		try {
			if (hp.valClientsbttn()) {
				...
				Assert.assertTrue(true);
				logger.info("Home Page Client button verification Passed");

			} else {
				logger.info("Home Page Client button verification Failed");
				captureScreen(driver, "hpclientbttnTest");
				Assert.assertTrue(false);
			}
		} catch (Exception e) {
